...
assert not output_wrapper.is_strict_json_schema() # This will return False, But not will make it True again, so this will pass
assert output_wrapper.json_schema() == Human.model_json_schema() # Both are equal since strict_json_schema=False (No Changes)
...
assert output_wrapper.json_schema() == Human.model_json_schema() # Now ths will pass, Because we have deleted the additionalProperties key
...
def __eq__(self: AgentOutputSchema, other: AgentOutputSchema | Any):
    ...
    return (
        ...
        self._strict_json_schema == other._strict_json_schema and
        self._is_wrapped == other._is_wrapped and
        self._output_schema == other._output_schema
    )
...
strict_true._strict_json_schema = False # Assign False to _strict_json_schema private attribute

print("Are Both Equal Now?", strict_false == strict_true, '\n') # Are Both Equal Now? True
