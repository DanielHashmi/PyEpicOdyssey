from agents import Agent, Runner, RunConfig, ModelProvider, OpenAIChatCompletionsModel, AsyncOpenAI    
...
class CustomConsoleSpanExporter(TracingExporter):      
    ...
    def export(self, items):
        for item in items:
            if hasattr(item, 'export') and item.export().get('span_data',{}).get('type','unknown') == 'generation':
    ...
    def _add_generation_data(self, tree, data):     
        if data.get('model'):    
            tree.add(f"[yellow]🎯 ACTUAL MODEL USED:[/yellow] [bold red]{data['model']}[/bold red]")
