...
class ComprehensiveRichConsoleSpanExporter(TracingExporter):  
    ...
    def _export_trace(self, trace: Trace):  
        ...
        trace_info = []  
        trace_info.append(f"[bold blue]Trace ID:[/bold blue] {trace_data.get('id', 'N/A')}")  
        trace_info.append(f"[bold blue]Name:[/bold blue] {trace_data.get('workflow_name', 'N/A')}")  
          
        if trace_data.get('group_id'):  
            trace_info.append(f"[bold blue]Group ID:[/bold blue] {trace_data['group_id']}")  
              
        if trace_data.get('metadata'):  
            trace_info.append(f"[bold blue]Metadata:[/bold blue] {json.dumps(trace_data['metadata'], indent=2)}")  
    ...
    def _export_span(self, span: Span[Any]):  
        ...
        if span_data.get('error'):  
            ...
            if error_info.get('data'):  
                error_tree.add(f"[red]Data:[/red] {json.dumps(error_info['data'], indent=2)}")  
    ...
    def _add_transcription_data(self, tree: Tree, data: dict):  
        ...
        if data.get('input'):  
            ...
            if isinstance(input_info, dict):  
                tree.add(f"[yellow]Input Format:[/yellow] {input_info.get('format', 'N/A')}")  
                if input_info.get('data'):  
                    tree.add(f"[yellow]Input Data:[/yellow] [Audio data present]")  
    ...
    def _add_speech_data(self, tree: Tree, data: dict):  
        ...
        if data.get('output'):  
            ...
            if isinstance(output_info, dict):  
                tree.add(f"[yellow]Output Format:[/yellow] {output_info.get('format', 'N/A')}")  
                if output_info.get('data'):  
                    tree.add(f"[yellow]Audio Output:[/yellow] [Audio data present]")  
